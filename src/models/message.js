...
chatMessageSchema.statics.createPostInChatRoom = async function (chatRoomId, message, postedByUser) {
  try {
    ...
    const aggregate = await this.aggregate([
      ...
      { $match: { _id: post._id } },
      // do a join on another table called users, and user
 ...
 * @return {Array} array of all chatroom messages that the user belongs to
 */
chatMessageSchema.statics.getMessagesByRoomId = async function (roomId) {
  try {
    const messages = await this.find({ chatRoomId: { $all: [roomId] } });
    return messages;
  } catch (error) {
    throw error;
  }
